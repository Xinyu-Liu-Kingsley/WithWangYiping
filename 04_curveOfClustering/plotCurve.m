...
ave = zeros(1,linesNum);
for i = 1:linesNum
[~,tmpline_i] = size(line{i});
for j = 1:linesNum
    [~,tmpline_j] = size(line{j});
    dist = zeros(tmpline_i,tmpline_j);
    if(j ~= i)
        for ii = 1:tmpline_i   
            for jj = 1:tmpline_j
        ...
        distance_point_to_line_min = min(dist,[],2);
        ave(j) = sum(distance_point_to_line_min)/tmpline_i;
    end
end
    distance{dist_idx} = ave;
    dist_idx = dist_idx+1;
end
